...
public class BlueThermalPrinterPlugin implements MethodCallHandler, RequestPermissionsResultListener {
  ...
  public void onMethodCall(MethodCall call, Result rawResult) {
    ...
    switch (call.method) {
      ...
      case "printBarcode":
        if (arguments.containsKey("textToBarcode")) {
          String textToBarcode = (String) arguments.get("textToBarcode");
          int width = (int) arguments.get("width");
          int height = (int) arguments.get("height");
          int align = (int) arguments.get("align");
          printBarcode(result, textToBarcode, width, height, align);
        } else {
          result.error("invalid_argument", "argument 'textToBarcode' not found", null);
        }
        break;
  ...
  private void printQRcode(Result result, String textToQR, int width, int height, int align) {
    ...
    try {
      ...
      printImage(bitMatrix,result,align);
//      int widthQrcode = bitMatrix.getWidth();
//      int heightQrcode = bitMatrix.getHeight();
//      Bitmap bmp = Bitmap.createBitmap(widthQrcode, heightQrcode, Bitmap.Config.RGB_565);
//      for (int x = 0; x < widthQrcode; x++) {
//        for (int y = 0; y < heightQrcode; y++) {
...
//      }
  ...
  private void printBarcode(Result result, String textToBarcode, int width, int height, int align) {
    MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
    if (THREAD == null) {
      result.error("write_error", "not connected", null);
      return;
    ...
    try {
      ...
      BitMatrix bitMatrix = multiFormatWriter.encode(textToBarcode, BarcodeFormat.CODE_128, width, height);
      printImage(bitMatrix,result,align);
...
///////////////////////////////////////////////////////////////////////////
  ...
  public void printImage(BitMatrix image,Result result,int align) {
    try {
      BitMatrix imageBits =image;
      ...
      THREAD.write(INITIALIZE_PRINTER);
      THREAD.write(setLineSpacing24Dots);
      ...
      while (offset < image.getHeight()) {
        ...
        THREAD.write(selectBitImageModeCommand);
        ...
        for (int x = 0; x < image.getWidth(); ++x) {
          ...
          for (int k = 0; k < 3; ++k) {
            ...
            for (int b = 0; b < 8; ++b) {
              ...
              boolean v = false;
              if (i < imageBits.getWidth()*imageBits.getHeight()) {
                v = imageBits.get(x,y);
              }
        ...
        THREAD.write(imageDataLine);
        offset += 24;
        THREAD.write(PRINT_AND_FEED_PAPER);
      }
      THREAD.write(setLineSpacing30Dots);
    } catch (Exception ex) {
      Log.e(TAG, ex.getMessage(), ex);
      result.error("write_error", ex.getMessage(), exceptionToString(ex));
    }
