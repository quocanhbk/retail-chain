...
Route::prefix('/admin')->group(function() {
    Route::prefix('/auth')->group(function() {
        ...
        Route::post('/refresh', [Admin::class, 'refreshToken']);
    });
});

Route::prefix('/auth')->group(function() {
    Route::post('/login', [Employee::class, 'login']);
    Route::post('/logout', [Employee::class, 'logout']);
    Route::post('/refresh', [Employee::class, 'refreshToken']);
});

Route::middleware(['jwt-verify'])->group(function () {
    Route::prefix("/admin")->group(function () {
        Route::middleware(['only-owner'])->group(function () {
            Route::get('/me', [Admin::class, 'getInfo']);

            Route::prefix('/employee')->group(function() {
                Route::post('/', [Admin::class, 'createEmployee']);
                Route::patch('/', [Admin::class, 'updateEmployee']);
                Route::get('/{branch_id}', [Admin::class, 'getEmployees']);
            });

            Route::prefix("/branch")->group(function () {
                Route::get('/', [Branch::class, 'getBranches']);
                Route::post('/', [Branch::class, 'create']);
            });
        });
    });

    Route::middleware(['only-employee'])->group(function () {
        Route::get('/me', [Admin::class, 'getInfo']);
    });
});
