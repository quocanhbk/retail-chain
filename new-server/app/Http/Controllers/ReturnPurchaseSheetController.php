...
class ReturnPurchaseSheetController extends Controller
{
    ...
    public function create(Request $request) {
        ...
        $rules = [
            ...
            'purchase_sheet_id' => ['required', 'integer', Rule::exists('purchase_sheets', 'id')->where('branch_id', $branch_id)],
            ...
            'items.*.item_id' => ['required', 'integer', Rule::exists('purchase_sheet_items', 'item_id')->where('purchase_sheet_id', $data['purchase_sheet_id'] ?? 0)],
        ...
        foreach($items as $item) {
            ...
            if ($item_property->quantity < $item['quantity']) {
                return response()->json(['errors' => ['items' => ['Item ' . $item['item_id'] . ' does not have enough quantity']]], 400);
            }

            // get all return purchase sheet items of that item
            $return_purchase_sheet_items = ReturnPurchaseSheetItem::with('returnPurchaseSheet')->where('item_id', $item['item_id'])
            ...
            $purchase_sheet_item = PurchaseSheetItem::where('item_id', $item['item_id'])->where('purchase_sheet_id', $data['purchase_sheet_id'])->first();
        ...
        foreach ($items as &$item) {
            ...
            $item['return_price'] = $item['return_price'] ?? ($item['return_price_type'] == 'percent' ? 100 : $item['price']);
            $final_price = $item['return_price_type'] == 'percent' ? $item['return_price'] / 100 * $purchase_sheet_item['price'] : $item['return_price'];
        ...
        if (!isset($data['code'])) {
            ...
            while (ReturnPurchaseSheet::where('code', $code)->where('branch_id', $branch_id)->exists()) {
        ...
        foreach ($items as $item) {
            ...
            $new_base_price = $new_quantity > 0 ? max(($item_property->price * $item_property->quantity - $item['total']) / $new_quantity, 0) : 0;
    ...
    public function getReturnPurchaseSheet(Request $request, $id) {
        $branch_id = Auth::user()->employment->branch_id;
        $return_purchase_sheet = ReturnPurchaseSheet::with('employee', 'branch', 'supplier', 'returnPurchaseSheetItems.item')
            ->where('branch_id', $branch_id)
            ->where('id', $id)
            ->first();
        ...
        $return_purchase_sheet->items = ReturnPurchaseSheetItem::where('return_purchase_sheet_id', $return_purchase_sheet->id)->get();
        return response()->json([
            'return_purchase_sheet' => $return_purchase_sheet,
        ]);
    }
    ...
    public function getReturnableItems(Request $request, $purchase_sheet_id) {
        ...
        $previous_return_purchase_sheets = ReturnPurchaseSheet::with('returnPurchaseSheetItems')->where('purchase_sheet_id', $purchase_sheet_id)->get();
        ...
        foreach ($purchase_sheet->purchaseSheetItems as $purchase_sheet_item) {
            ...
            $item_base_price = ItemProperty::where('item_id', $item->id)->where('branch_id', $branch_id)->first()->base_price;
    ...
    public function update(Request $request, $id) {
        ...
        $data['id'] = $id;
        $rules = [
            'id' => ['required', 'integer', Rule::exists('return_purchase_sheets', 'id')->where('branch_id', $branch_id)->where('employee_id', Auth::user()->id)],
            'discount' => ['nullable', 'numeric', 'min:0'],
            'discount_type' => ['nullable', 'string', 'max:255'],
            'paid_amount' => ['required', 'numeric', 'min:0'],
            'note' => ['nullable', 'string', 'max:255'],
        ...
        $items = $data['items'];
        foreach ($items as &$item) {
            $final_price = $item['return_price_type'] == 'percent' ? $item['return_price'] / 100 * $item['price'] : $item['return_price'];
            $item['total'] = $item['quantity'] * $final_price;
        }
        ...
        foreach ($items as $item) {
            ...
            $previous_return_purchase_sheet_item = ReturnPurchaseSheetItem::where('return_purchase_sheet_id', $return_purchase_sheet->id)
            ...
            if ($previous_return_purchase_sheet_item) {
                ...
                $old_base_price = ($item_property->base_price * $item_property->quantity + $previous_return_purchase_sheet_item->total) / $old_quantity;
                $item_property->update([
    ...
    public function delete(Request $request, $id) {
        ...
        foreach($return_purchase_sheet->returnPurchaseSheetItems as $return_purchase_sheet_item) {
            $item_property = ItemProperty::where('item_id', $return_purchase_sheet_item->item_id)->where('branch_id', $branch_id)->first();
            $old_quantity = $item_property->quantity + $return_purchase_sheet_item->quantity;
            $old_base_price = ($item_property->base_price * $item_property->quantity + $return_purchase_sheet_item->total) / $old_quantity;
            $item_property->update([
                'quantity' => $old_quantity,
                'base_price' => $old_base_price,
            ]);
