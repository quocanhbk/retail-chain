...
class RefundController extends Controller
{
    public function create(Request $request) {
        ...
        $rules = [
            ...
            'items.*.return_item_id' => ['required', 'integer', Rule::exists('return_items', 'id')->where('branch_id', $branch_id)],
        ...
        if (!isset($data['code'])) {
            ...
            while (RefundSheet::where('code', $code)->where('branch_id', $branch_id)->exists()) {
                $refund_sheets_count++;
                $code = 'REF' . str_pad($refund_sheets_count + 1, 6, '0', STR_PAD_LEFT);
            }
        ...
        foreach ($data['items'] as $item) {
            ...
            if (isset($item['resellable']) && $item['resellable']) {
                $item_property = ItemProperty::where('item_id', $invoice_item->item_id)->where('branch_id', $branch_id)->first();
    ...
    public function getRefundSheet(Request $request, $id) {
        ...
        $refund_sheet = RefundSheet::with(['branch', 'employee', 'invoice', 'refundItems'])->where('branch_id', $branch_id)
            ->where('id', $id)
            ->first();
