...
class QuantityCheckingSheetController extends Controller
{
    public function create(Request $request) {
        ...
        $rules = [
            ...
            'items.*.item_id' => ['required', 'integer', Rule::exists('item_properties', 'item_id')->where('branch_id', $branch_id)],
        ...
        if (!isset($data['code'])) {
            ...
            while (QuantityCheckingSheet::where('code', $code)->where('branch_id', $branch_id)->count() > 0) {
                $return_sheet_count++;
                $code = 'QS' . str_pad($return_sheet_count + 1, 6, '0', STR_PAD_LEFT);
            }
            $data['code'] = $code;
        ...
        foreach($items as &$item) {
            $item_property = ItemProperty::where('branch_id', $branch_id)->where('item_id', $item->id)->first();
    ...
    public function getQuantityCheckingSheets(Request $request) {
        ...
        $quantity_checking_sheets = QuantityCheckingSheet::with(['employee'])->where('branch_id', $branch_id)
    ...
    public function getQuantityCheckingSheet(Request $request, $id) {
        $branch_id = Auth::user()->employment->branch_id;
        $quantity_checking_sheet = QuantityCheckingSheet::with(['employee', 'branch', 'quantityCheckingItems'])->where('id', $id)
