...
const EmployeeCard = ({ data }: EmployeeCardProps) => {
	return (
		<Link href={`/admin/manage/employee/${data.id}`}>
			<Flex key={data.id} rounded="md" align="center" w="full" cursor="pointer" _hover={{ bg: "background.secondary" }} px={2} py={2}>
				<Avatar size="xs" src={getEmployeeAvatar(data.avatar_key)} alt={data.name} mr={2} />
				<Text w="15rem" isTruncated mr={2} flexShrink={0}>
					{data.name}
				</Text>
				<Text fontSize={"sm"} color={"text.secondary"} w="15rem" isTruncated flexShrink={0} mr={2}>
					{data.email}
				</Text>
				<Text fontSize={"sm"} color={"text.secondary"} w="8rem" isTruncated flexShrink={0}>
					{data.phone}
				</Text>
